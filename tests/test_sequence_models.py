...
try:
    ...
    from genatator_core import model_builders
...
class PlainTranscriptClassifierTests(unittest.TestCase):
    def test_gena_and_moderngena_pool_cls(self):
        ...
        for backbone_kind in ("gena", "moderngena"):
            with self.subTest(backbone_kind=backbone_kind):
                ...
                with patch(
                    "genatator_core.sequence_models.HiddenStateBackbone",
                ...
                with torch.no_grad():
                    model.classifier.weight.copy_(torch.tensor([[1.0, 0.0, 0.0]]))
    ...
    def test_plain_classifier_rejects_a_missing_cls(self):
        backbone = _EchoBackbone()
        with patch(
            "genatator_core.sequence_models.HiddenStateBackbone", return_value=backbone
        ):
